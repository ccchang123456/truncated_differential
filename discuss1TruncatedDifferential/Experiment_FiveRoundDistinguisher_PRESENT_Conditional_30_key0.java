...
// test new extended five-round distinguisher with present s-box: D1 is inactive in the condition that D0 is inactive
// we use 100 threads to test separately, take key0 as an example: key0 represents the 0th group of random key experiments using 2^30 ¦Ä-sets
public class Experiment_FiveRoundDistinguisher_PRESENT_Conditional_30_key0 {
	...
	public static int DELTASETSIZE = (int) Math.pow(2, 30);
	...
	public static int[] getZeroDifferentialByOneDeltaSetCipherText(int[][] ciphertext) {
		...
		for(int i = 0; i < ciphertext.length-1; i++) {
			for(int j = i+1; j < ciphertext.length; j++) {
				...
				int num0_0 = ciphertext[i][0] ^ ciphertext[j][0];
				int num0_5 = ciphertext[i][5] ^ ciphertext[j][5];
				int num0_10 = ciphertext[i][10] ^ ciphertext[j][10];
				int num0_15 = ciphertext[i][15] ^ ciphertext[j][15];
				
				int num1_1 = ciphertext[i][1] ^ ciphertext[j][1];
				int num1_6 = ciphertext[i][6] ^ ciphertext[j][6];
				int num1_11 = ciphertext[i][11] ^ ciphertext[j][11];
				int num1_12 = ciphertext[i][12] ^ ciphertext[j][12];
				
				if((num0_0 == 0) && (num0_5 == 0) && (num0_10 == 0) && (num0_15 == 0)) {
					count[0]++;
					if((num1_1 == 0) && (num1_6 == 0) && (num1_11 == 0) && (num1_12 == 0)) {
						count[1]++;
					}
				}
